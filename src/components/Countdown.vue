<template>
  <div class="min-h-screen flex justify-center items-center bg-gray-900">
    ...
    >
      <h1 class="text-2xl md:text-4xl mb-4 animate-fade-in-down">
        Sidang TA 1
      </h1>
      <div
      ...
      >
        ...
        >
          <p
            :class="
              displayMode === 'minutes' || displayMode === 'seconds'
      ...
      <div class="mt-2 md:mt-4 flex justify-center items-center">
        <label
          class="block text-white font-bold mb-2 md:hidden"
          for="displayMode"
          >Choose Mode:</label
        >
        <select
...
export default {
      ...
      switch (this.displayMode) {
        case "hours":
          return [
            {
              label: "Hours",
              value: String(Math.floor(totalHours))
                .padStart(2, "0")
                .toLocaleString(),
            },
            { label: "", value: ":" },
            {
              label: "Minutes",
              value: String(this.timeUnits[2].value).padStart(2, "0"),
            },
            { label: "", value: ":" },
            {
              label: "Seconds",
              value: String(this.timeUnits[3].value).padStart(2, "0"),
            },
          ];
        case "minutes":
          return [
        ...
        case "seconds":
          return [{ label: "Seconds", value: totalSeconds.toLocaleString() }];
        default:
  ...
  methods: {
    calculateTargetTime() {
      ...
      // update the target date to tomorrow
      if (new Date().getTime() > target.getTime()) {
        target.setDate(target.getDate() + 1);
        console.log("Target time recalculated:", target);
      }
    ...
    updateTime() {
      ...
      this.timeUnits[0].value = Math.max(
        0,
        Math.floor(distance / (1000 * 60 * 60 * 24))
      );
      this.timeUnits[1].value = Math.floor(
        (distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      this.timeUnits[2].value = Math.floor(
